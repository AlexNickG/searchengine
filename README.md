...
Проект разработан на Spring Boot Framework с использованием 
библиотек лемматизаторов.
В файле настроек проекта application.yaml задается перечень сайтов для 
индексации.
Индексация осуществляется при помощи ForkJoinPool.
Данные индексации сохраняются в базу данных MySQL.
